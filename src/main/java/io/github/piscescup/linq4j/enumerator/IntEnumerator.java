...
import io.github.piscescup.linq4j.core.InternalIntEnumerable;
...
/**
 * An enumerator over a sequence of primitive {@code int} values.
 *
 * <p>{@code IntEnumerator} is the primitive specialization of
 * {@link Enumerator} for {@code int} values. It provides cursor-based
 * enumeration semantics through {@link #moveNext()} and {@link #current()}
 * while avoiding boxing each element into an {@link Integer}.</p>
 *
 ...
 * has been reached.</p>
 *
 * <p>Unlike {@code Enumerator<Integer>}, this interface exposes values
 * directly as primitive {@code int}s. It also deliberately does not extend
 * {@code Iterator<Integer>}, because the {@link java.util.Iterator} contract
 * would require values to be represented as {@link Integer} objects and
 ...
 *
 * @apiNote
 * An {@link InternalIntEnumerable} can be converted into an
 * {@code IntEnumerator} by using the
 * {@link InternalIntEnumerable#enumerator()} method.
 *
 * <p>If interoperability with APIs requiring boxed values is needed,
 * callers should explicitly convert the corresponding
 * {@link IntEnumerable} to a reference-type enumerable rather than relying
 * on implicit boxing during primitive enumeration.</p>
 *
 * @author REN YuanTong
 * @since 1.0.0
 */
public interface IntEnumerator extends AutoCloseable {
    ...
    /**
     * Returns the primitive {@code int} value at the current cursor position.
     *
     ...
     *
     * <p>Elements are supplied directly as primitive {@code int} values and
